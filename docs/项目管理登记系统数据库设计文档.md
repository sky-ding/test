...
当前业务接口已统一为 v1 矩阵契约（`dept_groups + cells`）。新环境由应用 `create_all` 或 DBA 执行 `backend/migrations/001_relational_schema.sql` 建表即可，**不再提供**旧 `manpower_allocations` 行存表的迁移脚本。
...
旧版 `manpower_allocations` 行存表及配套迁移脚本已从代码库移除。新部署请直接使用 §16 / `backend/migrations/001_relational_schema.sql` 中的矩阵三表结构。
