...
**产品名称**：项目管理登记（单机 Web 工具）  
**文档版本**：1.0  
**对应代码库**：项目根目录 `index.html`（单页应用）  
...
面向项目管理场景的 **浏览器端登记与查看工具**，在 **无后端、无登录** 的前提下，将项目阶段状态、部门人力、风险与 PM 操作指引集中在一套界面中，数据默认持久化在 **本机浏览器 localStorage**，适合单机演示、轻量台账与权限约定演示。

### 1.2 目标用户

| 角色 | 说明 |
|------|------|
| **管理员** | 可维护项目树、部门结构、各页登记数据与指引内容，并可执行「保存到本机」。 |
...
| 保存粒度 | 人力、风险、阶段 **分按钮保存** 至不同 localStorage 键；指引使用独立 `pmGuideData` 键。 |
...
- **持久化**：`PM-tool-manpower-v1`（含 `data`、`deptGroups`）。
...
```mermaid
flowchart LR
  A[打开 index.html] --> B[loadSavedData 加载各 localStorage]
  B --> C[同步人力结构到阶段表规则]
  C --> D[渲染各 Tab 默认页]
  D --> E{用户切换 Tab}
  E --> F[编辑数据]
...
4. 点击「保存」将 `data` 与 `deptGroups` 写入 `PM-tool-manpower-v1`。

### 4.4 风险登记与排序

1. 管理员「+ 新增风险」生成一行（含默认登记时间等）。  
2. 填写各字段；表头点击切换排序键与方向。  
3. 「保存」前从 DOM 同步回 `riskRows`，再写入 `PM-tool-risk-v1`。
...
| 类别 | 说明 |
|------|------|
| 运行环境 | 现代浏览器；需联网加载 Chart.js CDN。 |
| 安全 | 无服务端校验；链接打开新标签 `noopener`；指引链接限制 http/https。 |
| 容量 | 受 localStorage 配额限制（通常约 5MB 级，依浏览器而定）。 |
| 多设备 | 数据仅存本机浏览器，不跨设备同步。 |
...
1. **阶段状态分析**：界面已预留，结论生成（如 AI）未实现。  
2. **通用设置**：仅占位文案。  
3. **协作与审计**：无多用户、无操作日志、无服务端备份。  
4. **导出/导入**：当前以浏览器存储为主，未内置 JSON 导出导入 UI（可作为后续 PRD 条目）。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.0 | 2026-04-11 | 初版，基于当前 `index.html` 实现梳理 |
