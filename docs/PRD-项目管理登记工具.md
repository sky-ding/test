...
面向项目管理场景的 **浏览器端登记与查看工具**，将项目阶段状态、部门人力与风险集中在一套界面中。**需先通过服务端登录**；**管理员 / 普通用户** 由 **登录账号在数据库中的角色** 决定（不再在设置里本地切换身份）。
...
| 角色 | 说明 |
|------|------|
| **管理员** | 登录账号角色为 `admin`：可维护项目树、部门结构、各页登记数据，可执行各页「保存」；可在 **设置 → 人员账号** 中维护用户列表（与后端 `/api/v1/users` 一致）。 |
| **普通用户（查看者）** | 登录账号角色为 `viewer`：可浏览各页数据、打开分析图表；不可编辑登记数据、不可保存、不可管理用户。 |

### 1.3 核心价值

- **统一入口**：阶段、人力、风险、设置多 Tab 切换。  
...
- **权限管理**：展示 **当前登录用户名与角色**（管理员/普通用户）说明文案；功能菜单权限说明表（阶段、人力、风险、设置）。  
- **人员账号**（仅管理员可见）：调用后端用户接口，支持创建用户、调整角色与状态等；普通用户见占位说明。  
- **退出登录**：与会话清除、跳转登录页一致（见 3.1）。  
- **持久化**：`PM-tool-app-settings-v1` 为应用侧预留键（当前为轻量占位写入）；**登录角色不以 localStorage 切换为准**，以服务端会话与 `/auth/me` 为准。
