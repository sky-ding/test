...
**项目名称**：项目管理登记（Web）  
**文档版本**：1.6  
**对应实现**：前端 [`frontend/index.html`](../frontend/index.html)、[`frontend/login.html`](../frontend/login.html)；后端 [`backend/`](../backend/)（FastAPI + SQLAlchemy + **MySQL（团队推荐）或 SQLite（本地）** + Starlette Session + bcrypt）  
...
- **前端**：无构建工具；主应用 **`frontend/index.html`**（内联 CSS + 单段主脚本 **`pmBootstrap`**）；**`frontend/login.html`** 独立登录页（`fetch` + `credentials: 'include'`，带超时）。  
...
- **持久化（关系型主线）**  
  - **浏览器**：`localStorage` 仅用于离线降级、列宽与轻量偏好。  
  - **服务端**：**FastAPI** 以关系型表保存项目树、阶段状态、人力矩阵、项目风险与用户账号；**GET 需登录，写操作需管理员**。持久化介质由配置决定：**`PM_MYSQL_*` 齐全时为 MySQL**，否则为 **`backend/data/app.db`（SQLite）**。
...
flowchart TB
  ...
  subgraph Server["backend"]
    ...
    DB[(MySQL_or_SQLite relational tables)]
  end
...
- **启动顺序（摘要）**：脚本加载时 **尽早** `bindPmLogoutButtons()`；`pmBootstrap`：`await fetchAuthMe()`（含 **Abort 超时**，避免 `/auth/me` 永久挂起）→ 再次绑定退出按钮（幂等）→ **尽早**注册 Tab 切换与删除弹窗键盘无障碍 → 执行 `loadRegistryData`、各 `init*Ui`、渲染与设置面板。
...
| `backend/data/` | 本地 SQLite 时存放 `app.db`；使用 MySQL 时可为空或仅存非库文件（`app.db` 通常不入库） |
| `docs/` | PRD、TDD、操作指南 |
...
| ORM / DB | **SQLAlchemy 2**、**MySQL（pymysql）或 SQLite** | 关系型业务表 + `users` |
...
| `app/models.py` | **`User`**；历史 `RegistryEntry`（仅归档兼容，不作为正式业务入口） |
| `app/schemas.py` | 登录与用户 CRUD 模型；旧快照模型仅作兼容 |
...
| `app/registry_store.py` | 历史快照存储辅助；正式业务入口不再注册旧快照路由 |
| `app/routers/auth.py` | `POST /login`、`POST /logout`、`GET /me` |
| `app/routers/users.py` | 用户列表/创建/更新/删除（**管理员**；保护最后一名活跃管理员） |
| `app/routers/programs.py` / `phase_assessments.py` / `manpower_allocations.py` / `project_risks.py` | 正式关系型业务 API |
...
1. `saveManpowerData` → `PUT /api/v1/manpower-allocations?year=&period=`，body 为 `cells`。  
2. `savePhaseData` → `PUT /api/v1/phase-assessments?year=`，body 含 `sub_project_id` 与 `period`。  
3. `saveRiskData` → `/api/v1/project-risks` 的 POST/PATCH/DELETE。

### 5.3 后端处理单次写入

1. Pydantic 校验 body。  
2. SQLAlchemy 事务内写入/更新关系型表。  
3. 返回最新行或矩阵快照。
...
| `GET` | `/api/v1/programs/tree?year=` | 项目树 | **已登录** | 项目主数据 |
| `GET` | `/api/v1/phase-assessments?year=&period=` | `PhaseAssessmentOut[]` | **已登录** | 阶段月度列表 |
| `PUT` | `/api/v1/phase-assessments?year=` | `PhaseAssessmentUpsert` | **管理员** | 按 `sub_project_id + period` upsert |
| `GET` | `/api/v1/manpower-allocations?year=&period=` | `{ year, period, dept_groups, cells }` | **已登录** | 人力矩阵月度快照 |
| `PUT` | `/api/v1/manpower-allocations?year=&period=` | `{ cells: [...] }` | **管理员** | 批量 upsert 人力单元格 |
| `GET/POST/PATCH/DELETE` | `/api/v1/manpower-department-groups...` | 表头分组与列 | **GET 登录 / 写管理员** | 表头落库维护 |
| `GET/POST/PATCH/DELETE` | `/api/v1/project-risks...` | 风险行 | **GET 登录 / 写管理员** | 风险登记 |

迁移脚本：`backend/scripts/migrate_manpower_allocations_to_matrix.py`；旧行存清理：`backend/migrations/003_drop_legacy_manpower_allocations.sql`；冒烟测试：`pytest tests/test_manpower_matrix_integration.py`。
...
| 键名 | 载荷形状（摘要） |
|------|------------------|
| `PM-tool-manpower-v1` | 离线降级用 `{ data, deptGroups, savedAt }` |
| `PM-tool-phase-v1` | 离线降级用 `{ phaseData, savedAt }` |
| `PM-tool-risk-v1` | 离线降级用 `{ riskRows, savedAt }` |
| `PM-tool-app-settings-v1` | 应用预留键（轻量占位）；**登录角色不依赖本键**；无独立「通用设置」界面 |
...
- **前端**：`escapeHtml`、`save*` 异常提示；`login.html` 对登录请求 **超时**；`renderRiskTable` 对缺失 `tbody` 做保护；`pmLogout` **先 await** `POST /auth/logout` 再 `replace` 至 `login.html?logged_out=1`（`checkLoggedIn` 遇该参数或 `sessionStorage.pm_logged_out` 时不自动回跳 `index.html`）。  
- **后端**：会话签名密钥 **`PM_SESSION_SECRET`** 生产必填；业务写接口 **管理员** 校验；用户模块保护 **最后一名活跃管理员**。  
- **MySQL（生产）**：多实例共用同一库；备份与权限由 DBA/运维策略管理；连接池已启用 `pool_pre_ping`。  
- **SQLite（本地）**：单文件，注意备份与并发写入规模（仅建议本机开发）。  
...
3. **后端**：多租户、可选 JWT、PostgreSQL、扩展设置等 API。  
...
| 1.3 | 2026-05-12 | **团队生产推荐 MySQL**（`PM_MYSQL_*`）；明确与本地 SQLite 二选一；部署包不携带 `app.db` 的说明与 `check_db` 冒烟脚本 |
| 1.4 | 2026-05-13 | 与 PRD 对齐：设置内无「通用设置」子页；`PM-tool-app-settings-v1` 表述更新 |
| 1.5 | 2026-05-13 | **人力矩阵 v2**（`manpower_cells`）、v1 GET 合成与 v1 PUT 410；pytest 冒烟用例说明 |
| 1.6 | 2026-05-14 | 全面关系型改造：人力矩阵收敛到 v1 契约，表头 CRUD 落库，旧行存与 registry 正式业务接口下线 |
