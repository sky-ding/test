...
- **持久化（双轨）**  
  - **浏览器**：`localStorage` 为人力、阶段、风险、列宽等的主要实现。  
  - **服务端**：**FastAPI** 提供 `/api/v1/manpower`、`/phase`、`/risk` 的 `GET`/`PUT`（**GET 需登录，PUT 需管理员**），JSON 与 localStorage 根对象形状一致，存入 **`registry`** 表；**`users`** 表存登录账号。持久化介质由配置决定：**`PM_MYSQL_*` 齐全时为 MySQL**，否则为 **`backend/data/app.db`（SQLite）**。
...
flowchart TB
  ...
  subgraph Server["backend"]
    ...
    DB[(MySQL_or_SQLite users plus registry)]
  end
...
| ORM / DB | **SQLAlchemy 2**、**MySQL（pymysql）或 SQLite** | `users` + `registry` |
...
#### 6.1.1 关系型登记与人力矩阵（与 `registry` 并存）

当使用规范化项目树（`/api/v1/programs/tree`）时，阶段 / 风险 / 人力走关系型表。人力矩阵（见 `docs/项目管理登记系统数据库设计文档.md`、`migrations/002_manpower_matrix.sql`）启用后：

| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/v2/manpower-matrix?year=&period=` | **已登录**；返回 `dept_groups` + `cells` |
| `PUT` | `/api/v2/manpower-matrix?year=&period=` | **管理员**；body `{ cells: [{ sub_project_id, column_id, allocation }] }`，按单元格 upsert |
| `GET` | `/api/v1/manpower-allocations?year=&period=` | 若该年已存在 `manpower_department_groups` 行，则从 **`manpower_cells` 合成** 为旧 `department`/`role` 行列表 |
| `PUT` | `/api/v1/manpower-allocations?year=&sub_project_id=&period=` | 同上矩阵年返回 **410 Gone**（须改用 v2） |

迁移脚本：`backend/scripts/migrate_manpower_allocations_to_matrix.py`；冒烟测试：`pytest tests/test_manpower_matrix_integration.py`。
