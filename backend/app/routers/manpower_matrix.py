"""人力矩阵 v2：dept_groups + cells（按设计文档）。"""
...
router = APIRouter(prefix="/manpower-matrix", tags=["manpower-matrix"])


def _build_matrix_response(db: Session, y: int, p: str) -> ManpowerMatrixResponse:
    groups = list(
        db.scalars(
...
def get_manpower_matrix(
    ...
    return _build_matrix_response(db, y, p)


@router.put("", response_model=ManpowerMatrixResponse)
def put_manpower_matrix(
    _admin: AdminUser,
